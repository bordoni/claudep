...
- Windows groundwork: `claudep init` stops with Developer Mode instructions when Windows refuses to create a symlink; `claudep alias` writes `<command>.cmd` next to the sh shim; an npm-installed `claude.cmd` is launched through cmd.exe and `claude.exe` is preferred when both exist.
