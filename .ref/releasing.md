...
Releases go to npm and to GitHub Packages, both as `@bordoni/claudep`, and to a GitHub Release whose notes are the changelog section. The installed command is `claudep`. One command cuts a release; a tag push publishes it. Tags and GitHub Releases are the bare version, `0.1.0`, never `v0.1.0`.
...
## What the workflow does on a version tag
...
| `github-release` | npm | `gh release create <version>` with `--notes-file` from `bun scripts/changelog.ts extract`. |
...
Then push the tag: `git push origin 0.1.0`. The workflow verifies, sees 0.1.0 on npm and skips that job, publishes the GitHub Packages mirror and creates the GitHub Release.
...
- The tag is wrong and nothing was published yet: `git tag -d X.Y.Z && git push origin :refs/tags/X.Y.Z`, then `git revert` the release commit if it was pushed. Never move a tag that npm has already published.
