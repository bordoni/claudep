...
- `test/completion.test.ts`: the scripts from `claudep completion` in real shells, each case gated on the shell being installed. `-n` parse checks for zsh, bash and fish; bash and fish are driven to produce candidates (commands, profile names from a fake root, flags, `--shell` values); zsh is loaded under `compinit -u -D` and checked to register `_claudep`, because `_arguments` cannot run outside a completion widget. Skipped on Windows.
