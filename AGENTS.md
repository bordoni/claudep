...
- Runs on macOS, Linux and Windows (PowerShell 5.1 and 7, Git Bash). Shell hooks exist for zsh, bash, fish and PowerShell. The Keychain check is darwin-only; elsewhere `doctor` checks for `.credentials.json`. cmd.exe is not a target. CI runs the suite on all three.
...
10. **Never put a subprocess or network call in the shell hook.** It runs on every directory change or prompt. Parameter expansion and builtins only; in the fish hook, builtins only, where every `(...)` is a `string` call; in the PowerShell hook, cmdlets only, and the only `&` is the saved `prompt`.
